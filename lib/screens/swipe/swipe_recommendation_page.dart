...
class _SwipeRecommendationPageState extends State<SwipeRecommendationPage> {
  ...
  Future<void> _addToBag(List<Product> products) async {
    if (_isProcessing || _currentIndex >= products.length) return;
    
    final product = products[_currentIndex];
    final cartProvider = context.read<CartProvider>();
    
    // İşlem başladığını işaretle (ama swipe etme, sadece bag'e ekle)
    setState(() {
      _isProcessing = true;
    });
    
    try {
      // Ürünü sepete ekle (await ile bekliyoruz)
      await cartProvider.addToCart(product);
      
      // Kullanıcıya bilgi ver (kullanıcı swipe ekranında kalacak)
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${product.title} added to shopping bag!'),
      ...
      // Hata durumunda kullanıcıya bilgi ver ve hatayı logla
      debugPrint('Error adding to cart: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error adding to bag: ${e.toString()}'),
      ...
      // İşlem bittiğinde işaretle (ama _currentIndex'i artırma, ürün swipe edilmedi)
      if (mounted) {
        setState(() {
          _isProcessing = false;
        });
              ...
              Expanded(
                child: uid == null
                    ...
                    : StreamBuilder<List<Product>>(
                        stream: productProvider.streamSwipeableProducts(),
